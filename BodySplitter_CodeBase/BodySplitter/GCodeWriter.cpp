...
lastX(0), lastY(0), lastZ(0), lastFeedrate(0), lastE(0), lastR(-100), lastG(-100), lastB(-100), retracted(false), m_ptrMachine(nullptr)
{
	...
	{
		std::cerr << "Error openinig" << outputFileName << std::endl;
	}
...
void GCodeWriter::startWriter() {
	...
	if (GET_BOOL("primeToolPurgeBin"))
	{
		...
		//double targetX = GET_DOUBLE("purgeLocationX");
		//double targetY = GET_DOUBLE("purgeLocationY");
		//updated by CG on Aug 5th 2018
		writeComment("TravelToPurgeBin");//:(" + std::to_string(targetX) + "," + std::to_string(targetY) + ")");
		writeTravelTo(0, 340, lastZ);
		writeTravelTo(0, 349.5, lastZ);
		writeTravelTo(GET_DOUBLE("purgeLocationX"), GET_DOUBLE("purgeLocationY"), lastZ);
		writeManualIssue(GET_DOUBLE("purgeLength"), GET_DOUBLE("purgeFeedrate"));
	}
}
...
void GCodeWriter::bufferLine(const std::string &text, const std::string &comment)
{
	...
	if (text[0] == 'T' && text[1] >= '0' && text[1] <= '9')
	{
		...
		if (firstTCommand==false && GET_BOOL("mCommand")) // freelancer latino
		{
			bufferText = Settings::getSingleton().getStringValue("mCommandText");// freelancer latino
			if (bufferText.back() != '\n') // freelancer latino
				bufferText += '\n'; // freelancer latino
		} // freelancer latino
	}
	...
	{
		if (hax_buffer.compare("") != 0)
		{
			bufferText += hax_buffer;
			hax_buffer = "";
		}
	}*/
	...
	// added by freelancer latino
	if (text[0] == 'T' && text[1] >= '0' && text[1] <= '9')
	{
		if (firstTCommand==false && GET_BOOL("purgeOnToolChange")) // freelancer latino
		{
			if (GET_BOOL("purgeAttribute"))
			{
				std::string result = Settings::getSingleton().getStringValue("onPurgeAttribute");
				bufferText += result;
				if (bufferText.back() != '\n')
					bufferText += '\n';
			}
		}
		bufferText += "G92 E" + std::to_string(lastE) + "\n";
		bufferText += "G1 X" + std::to_string(lastToolPosition.x) + " Y" + std::to_string(lastToolPosition.y) + " F12000\n";

		
		bufferText += "G1 Z" + std::to_string(lastToolPosition.z) + " F10800\n";
		bufferText += "G92 E0\n";
		double dt0 = Settings::GetSingleton().getDoubleValue("toolChangeIssueDistanceT0");
		double fr = Settings::getSingleton().getDoubleValue("issueFeedrateT0");
		bufferText += "G1 E" + std::to_string(dt0) + " F" + std::to_string(fr) + "\n";
		bufferText += "G92 E" + std::to_string(lastE) +"\n";
		

		/*
		hax_buffer = "G1 Z" + std::to_string(lastToolPosition.z) + " F10800\n";
		hax_buffer += "G92 E0\n";
		double dt0 = Settings::GetSingleton().getDoubleValue("toolChangeIssueDistanceT0");
		double fr = Settings::getSingleton().getDoubleValue("issueFeedrateT0");
		hax_buffer += "G1 E" + std::to_string(dt0) + " F" + std::to_string(fr) + "\n";
		hax_buffer += "G92 E" + std::to_string(lastE) + "\n";
		*/

		//lastE += Settings::GetSingleton().getDoubleValue("toolChangeIssueDistanceT0");
		//bufferText += "G1 E" + std::to_string(lastE) 
		//	+ " F" + std::to_string(Settings::getSingleton().getDoubleValue("issueFeedrateT0")) + "\n";
		firstTCommand = false;
	}
...
//Added by CG on aug 5th 2018
void GCodeWriter::writeManualIssue(volatile double IssueAmt, volatile double FeedRate)
{
...
void GCodeWriter::writeTool(uint16_t tool)
{
	...
	//bufferLine("T" + std::to_string(static_cast<int>(tool))); // commented by freelancer latino
	lastTool = tool;

	//if (Settings::getSingleton().getBoolValue("verbose"))
	//	std::cout << "writeTool called, GET_BOOL(purgeOnToolChange) = " << GET_BOOL("purgeOnToolChange") <<  " and GET_BOOL(purgeAttribute) = " << GET_BOOL("purgeAttribute") << std::endl;
	/*
	if (GET_BOOL("purgeOnToolChange")) // freelancer latino
	{
		if (GET_BOOL("purgeAttribute"))
		{
			std::string result = Settings::getSingleton().getStringValue("onPurgeAttribute");
...
void GCodeWriter::writeSoftwareColourPurge(const Colour &in)
{
	...
	double targetX = GET_DOUBLE("purgeLocationX");
	double targetY = GET_DOUBLE("purgeLocationY");
	writeComment("TravelToPurgeBin:writeSoftwareColourPurge"); //:(" + std::to_string(targetX) + ", " + std::to_string(targetY) + ")");
	//writeTravelTo(targetX, targetY, lastZ);
...
std::string GCodeWriter::getColourString(const Colour &in, bool hardwarePurge) const
{
	...
	{
		if (0)	// added by freelancer latino, for debugging purposes
		{
			static int ii = 0;
			ii++;
			...
			if (ii == 5)
			{
				Settings *p = &Settings::getSingleton();
				int n = p->getColours();
				FILE *f = fopen("c:\\temp\\BodySplitter\\test.txt", "wt");
				if (f)
				{
					fprintf(f, "%d\n", n);
					std::unordered_map<uint32_t, uint16_t> &cm = p->getColourToolMap();
					for (std::unordered_map<uint32_t, uint16_t>::iterator i = cm.begin(); i != cm.end(); i++)
					{
						fprintf(f, "color = %x (%d) tool = %d\n", i->first, i->first, i->second);
					}
					fclose(f);
				}
			}
		...
		// added by freelancer latino BECAUSE in = 0 sometimes, and we dont have such a color in the table
		Settings *p = &Settings::getSingleton();
		int bestID = -1;
		int smallest_err = 100000;
		if (p)
		{
			std::unordered_map<uint32_t, uint16_t> &cm = p->getColourToolMap();
			uint8_t r = in.getR<uint8_t>();
			...
			for (std::unordered_map<uint32_t, uint16_t>::iterator i = cm.begin(); i != cm.end(); i++)
			{
				Colour c = Colour (i->first);
				if (c == in)
				{
					bestID = i->second;
					break;
				}
				uint8_t R = c.getR<uint8_t>();
				uint8_t G = c.getG<uint8_t>();
				uint8_t B = c.getB<uint8_t>();
				int err = (R > r) ? R - r : r - R;
				err += (G > g) ? G - g : g - G;
				err += (B > b) ? B - b : b - B;
				if (err < smallest_err)
		...
		//uint16_t toolId = Settings::getSingleton().getToolByColour(in);	commented by freelancer latino due to the bug of in = 0 and the assert
		uint16_t toolId = bestID;
...
{
	if (lastTool < 0 || lastTool >= toolRetractionState.size())	// added by freelancer latino
		return;
	if (toolRetractionState[lastTool] != RETRACTION_TYPE::NO_RETRACTION)
		return;
